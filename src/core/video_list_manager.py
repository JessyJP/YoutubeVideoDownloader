...
class VideoListManager:
    ...
    def video_item_process_download(self, n, limits, outputdir, outputExt):
        # Define variables
        N = len(self.infoList)
    ...
    def downloadAllVideoItems(self, process_via_multithreading, limits, outputDir, outputExt):
        ...
        for n in range(N):
            ...
            if process_via_multithreading:
                t = threading.Thread(target=self.video_item_process_download, args=(n, limits, outputDir, outputExt))
                t.start()
                download_threads.append(t)            
            else:
                self.video_item_process_download(n, limits, outputDir, outputExt)
            #end
        #end
    ...
    def updateUI_ItemElementStatus(self, videoItem):
        pass
